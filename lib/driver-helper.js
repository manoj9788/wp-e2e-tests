...
export function eyesScreenshot( driver, eyes, pageName, selector ) {
	const date = new Date();
	console.log( `eyesScreenshot - ${pageName} - ${date}` );

//TODO - Add a conditional argument in here to execute the blur event.  Should probably construct
//	the tests so they don't need it, but it might be nice as an option at some point
...
//		driver.sleep( 1000 ).then( function() { // Wait 1 second to make sure everything's clean
	if ( selector ) {
		...
		if ( selector.using ) {
			return eyes.checkRegionBy( selector, pageName ).then( function() {
				console.log( new Date() );
			} );
		}

		return eyes.checkRegionByElement( selector, pageName ).then( function() {
			console.log( new Date() );
		} );
	}

	return eyes.checkWindow( pageName ).then( function() {
		console.log( new Date() );
	} );
//		} );
//	} );
